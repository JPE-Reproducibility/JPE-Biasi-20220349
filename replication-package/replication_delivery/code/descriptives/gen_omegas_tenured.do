...
*
* File:    gen_omegas_tenured.do
* Purpose: Create the alpha grid allowing omegas to differ by tenure status.
********************************************************************************
...
********************************************************************************
* Grid Search for Tenure-Specific Omega Parameters
********************************************************************************
...
gen Comb_untenured = Comb * (Exp1 + Exp2)
gen Comb_tenured = Comb * (1 - (Exp1 + Exp2))
    ...
    forvalues Alpha0 = 0.9(0.05)1.1 {
        ...
        foreach Alpha1 in 0 10000 30000 50000 75000 100000 200000 {
            ...
            foreach Alpha2 in 0 10000 30000 50000 75000 100000 200000 {
                ...
                replace wage = (`Alpha0' * mode) + ///
                    (`Alpha1' * Comb_untenured) + ///
                    (`Alpha2' * Comb_tenured) if district_code == `d'
...
********************************************************************************
* Save Tenure-Specific Grid and Wage-Deviation Inputs
********************************************************************************
...
sort district_code
merge m:1 district_code using `alpha'
drop _m
gen untenured = Exp1 + Exp2
bysort district_code: egen tot_unten = sum(untenured)
replace S_alpha1 = T_alpha1 if tot_unten == 0
replace S_alpha2 = T_alpha1 if tot_unten == 0
drop T_*

preserve
gen Wage = (S_alpha0 * mode) + (S_alpha1 * Comb_untenured) + ///
    (S_alpha2 * Comb_tenured)
replace Wage = `high' if wage > `high' & wage != .
replace Wage = `low' if wage < `low' & wage != .
gen S_dev = salary_round - Wage
keep id S_dev comb
sort id
save "$intdata/alphagrid_wages_8groups_tenured_JPE.dta", replace
restore



********************************************************************************
* Summary Statistics for Tenure-Specific Wage Deviations (Figure B5)
********************************************************************************

use "$intdata/alphagrid_wages_8groups_tenured_JPE.dta", clear
rename S_* SS_*
sort id
merge 1:1 id using "$intdata/alphagrid_wages_8groups_JPE.dta"
twoway scatter S_ SS_, ///
    xtitle("wage deviations, new (tenured distinction)") ///
    ytitle("wage deviations, old") ///
    xlabel(-40000(20000)40000) ylabel(-40000(20000)40000)
graph export "$out/figures/Figure_B5.png", replace

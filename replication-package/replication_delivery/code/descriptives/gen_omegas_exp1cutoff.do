...
*
* File:    gen_omegas_exp1cutoff.do
* Purpose: Create the alpha grid allowing omegas to differ by experience cutoff.
********************************************************************************
...
********************************************************************************
* Grid Search for Experience-Cutoff Omega Parameters
********************************************************************************
...
gen Comb_untenured = Comb * (Exp1)
gen Comb_tenured = Comb * (1 - (Exp1))
...
********************************************************************************
* Save Experience-Cutoff Grid and Wage-Deviation Inputs
********************************************************************************
...
drop _m
gen lowexp = Exp1 
bysort district_code: egen tot_lowexp = sum(lowexp)
replace S_alpha1 = T_alpha1 if tot_lowexp == 0
replace S_alpha2 = T_alpha1 if tot_lowexp == 0
drop T_*
...
keep id S_dev comb
sort id
save "$intdata/alphagrid_wages_8groups_exp1cutoff_JPE.dta", replace
restore


********************************************************************************
* Summary Statistics for Experience-Cutoff Wage Deviations (Figure B6)
********************************************************************************

use "$intdata/alphagrid_wages_8groups_exp1cutoff_JPE.dta", clear
rename S_* SS_*
sort id
merge 1:1 id using "$intdata/alphagrid_wages_8groups_JPE.dta"
twoway scatter S_ SS_, ///
    xtitle("wage deviations, new (exp = 1 cutoff)") ///
    ytitle("wage deviations, old") ///
    xlabel(-40000(20000)40000) ylabel(-40000(20000)40000)
graph export "$out/figures/Figure_B6.png", replace

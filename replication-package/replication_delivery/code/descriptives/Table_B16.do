...
*      Produces a CSV with 6 columns (3 elasticities x 2 counterfactuals)
*      Note: Baseline columns (cols 1 & 5 in Table B16) can be found in Table 7.
********************************************************************************
...
*-------------------------------------------------------------------------------
local baseline "$fortran_code/5_exitentry/baseline/result/"
local cf1 "$fortran_code/5_exitentry/new1_05/result/"
local cf2 "$fortran_code/5_exitentry/new1_1/result/"
local cf3 "$fortran_code/5_exitentry/new1_15/result/"
local cf4 "$fortran_code/5_exitentry/new2_05/result/"
local cf5 "$fortran_code/5_exitentry/new2_1/result/"
local cf6 "$fortran_code/5_exitentry/new2_15/result/"
* extensive margin happens here in period '2' which is t=1
local tp = 2

* Matrix to store results: 3 rows (tc, c1, c2) x 6 columns
matrix results = J(3, 6, .)

********************************************************************************
* Loop over 6 counterfactuals
********************************************************************************
forvalues i = 1/6 {
    
    di "Processing counterfactual `i' of 6..."
...
* New1 columns
gen double new1_elas05 = .
gen double new1_elas10 = .
gen double new1_elas15 = .

* New2 columns
gen double new2_elas05 = .
gen double new2_elas10 = .
gen double new2_elas15 = .

forvalues j = 1/3 {
    replace new1_elas05 = results[`j', 1] in `j'
    replace new1_elas10 = results[`j', 2] in `j'
    replace new1_elas15 = results[`j', 3] in `j'
    replace new2_elas05 = results[`j', 4] in `j'
    replace new2_elas10 = results[`j', 5] in `j'
    replace new2_elas15 = results[`j', 6] in `j'
}

* Format to 2 decimal places for display
format new1_elas05 new1_elas10 new1_elas15 ///
       new2_elas05 new2_elas10 new2_elas15 %9.2f
	   
foreach v of varlist new1_elas05 new1_elas10 new1_elas15 ///
                     new2_elas05 new2_elas10 new2_elas15 {
    gen str10 s_`v' = string(`v', "%9.2f")
    drop `v'
    rename s_`v' `v'
}

* Add note about where to find other Table B16 columns previously computed
local newobs = _N + 1
set obs `newobs'
replace variable = "Note: Baseline columns (columns 1 and 5 in Table B16) can be found in Table 7." in `newobs'

export delimited using "$out/tables/Table_B16.csv", replace

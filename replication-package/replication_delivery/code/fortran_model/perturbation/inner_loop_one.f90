  ...
  subroutine run_inner_loop_one(pr_model_match,model_match_copies, pr_model_Wgrid, &
    ...
    if (teachOff_decision == 1) then
       if (offer_eqbm == 1 .and. skip_inner_loop == 0) then
          !
          if(nround<=sum(incl,dim=1)+2  .or. purpose == -1) then
             theta = theta0
          else
             theta = theta_sofar
          endif
          ...
          if(nround==1 .and. purpose .ne. -1) then
             theta0 = best_theta
          endif
          theta       = best_theta
          ...
          if (purpose == -1) theta = theta0 !just in case, this makes sure it would start w/ right belief

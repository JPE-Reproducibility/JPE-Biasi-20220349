...
contains
  ...
  function objective_function(param0_in) result(L)
    ...
    double precision, allocatable, dimension(:,:) :: pr_model_match
    !integer, dimension(ncopyT,nteach,ndist)    :: model_match_copies
    integer, allocatable, dimension(:,:,:)    :: model_match_copies

    !integer,          dimension(nEconomy,nteach,ndist) :: sim_matches
    integer, allocatable, dimension(:,:,:) :: sim_matches

    !for validation purpose
    !double precision, dimension(nteach,  ndist) :: match_dbl
    double precision, allocatable, dimension(:,:) :: match_dbl
    !integer, dimension(ncopyT, nteach, ndist)   :: match_copies
    integer, allocatable, dimension(:,:,:)   :: match_copies
    ...
    allocate(pr_model_match(nteach,ndist))
    allocate(model_match_copies(ncopyT, nteach, ndist))
    allocate(sim_matches(nEconomy, nteach, ndist))
    allocate(match_dbl(nteach, ndist))
    allocate(match_copies(ncopyT, nteach, ndist))
   ...
   ! gamma_in = exp(gamma_in)
    ...
    if (purpose < 10) then
       ...
       print*, '--------- Equilibrium Simulation Finished ----------', t_period
       !if doing forward iter, need to save matches so we can make them incumbants next period
       if (T_max > 1) then
          do i=1, nteach
             ! inc_period saves the index of the incumbant district, i.e. one number for each teacher instead of dummy var
             inc_period(:,i) = maxloc(sim_matches(:,i,:),2)
             ! Checking inc_period is well behaved,
             ! XX can comment out later
             do k=1, nEconomy
                if (inc_period(k,i) <= 0 .or. inc_period(k,i) > ndist) then
                   print*, 'inc period zero or lower in obj, OR bigger than ndist', k, i, inc_period(k,i)
                   print*, sum(sim_matches(k,i,:))
                   call exit(1)
                end if
             end do
          end do
          print*, 'sim match sum check in obj', sum(sim_matches)/nEconomy, minval(sim_matches), maxval(sim_matches)
          print*, 'inc sum check in obj',  sum(inc_period)/nteach, minval(inc_period), maxval(inc_period)
       end if
    end if 
    

    ! Calculate our moments
    !    (with inc being economy-by-economy for foward iter, can't calclate this stuff anyways, and we don't need/want it)
    if (T_max == 1) then
...
!!!
    ...
    !dealloc nteach vars
    deallocate(pr_model_match, model_match_copies, sim_matches, match_dbl, match_copies)

...
!!!
  ...
  subroutine simulate_equilibrium(alpha_in, beta_in, gamma_in, sigma_in, sigmaD_in,&
    ...
    double precision, dimension(50, ntheta) :: econ_main_theta
    ...
    !   FOR "CASE" CODE NEED THIS TO BE >= except for first eqbm run. Also see line ~864
    if (purpose >= 10) then
    !if (purpose == 10) then
    ...
    do m=1, Neconomy
       ...
       !if (purpose == 10) then
          ...
          theta0      = econ_main_theta(economy_idx,:)
       end if
  ...
  function equilibrium_objective(arg1, arg2) result(D)
    ...
    double precision :: w1, w2, w3, w2a, w3a
    ...
    !calc likelihood D  (USING ALTERNATIVE OBJ FOR THIS EXERCISE, SEE NOTES)
    w1 = 10000
    w2a = 0.5
    w3a = 0.5
    w2 = 1
    ...
    if (size(arg1,1) .eq. ntheta) then
       ...
       D1 = w1 * sum((H_target - H_eqm_glob)**2) / (nteach * ndist)
       D2 = w2a * sum((sum(H_target*offer_eqm_glob,1) - T(1,:))**2) / ndist    !"truec"
       D3 = w3a * sum((sum(H_eqm_glob*offer_eqm_glob,1) - T(1,:))**2) / ndist  !"belc"
       D = D1 + D2 + D3

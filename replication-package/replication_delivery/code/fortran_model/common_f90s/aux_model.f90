...
contains
  ...
  subroutine calc_aux_parameters(pr_model_match,model_match_copies,aux_beta_est)
    ...
    double precision, allocatable, dimension(:,:) :: x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, y
    ...
    allocate(x13(nteach,ndist), y(nteach,ndist))
    ...
    ! if purpose >= 10:
       ...
       !Not necessary anyways at this stage! only truly needed for estimation
       mbeta_small = 0
       mbeta_big = 0
    end if ! purpose < 10
...
!
  ...
  subroutine calc_aux_other_parameters(pr_match, aux_other_out)
    ...
    do i=1, 5
       mom1(i,:) = sum(e_xd * spread(lambda_quintiles(i,:),2,4),1) / sum(lambda_quintiles(i,:))!82
       mom2(i,:) = sum(e_xd * spread(bt_quintiles(i,:),2,4),1)     / sum(bt_quintiles(i,:))!82
    end do

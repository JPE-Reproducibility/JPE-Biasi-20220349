...
contains
  ...
  subroutine read_instructions()
    ...
    read(1,*) h_tol_pct
    read(1,*) h_tol_dif
    read(1,*) rcost_experiment
    read(1,*) nomovecost_experiment
    read(1,*) NcopyT
    read(1,*) NcopyD
    read(1,*) Neconomy
    sim_nteach = NcopyT * nteach
    sim_ndist  = NcopyD * ndist
    !read(1,*) omega2_restriction (OLD, experimental)
    read(1,*) starting_economy
    !read(1,*) rvec_gradient !deprecated
    read(1,*) s_vec
    close(1)
    ...
    ! and for SE
    if (purpose == -1 .or. purpose == -2) then
       teachOff_decision = 1 
       offer_eqbm = 1
       skip_inner_loop = 1
    end if
    ...
    if (purpose > 10 .and. purpose <= 14) then
       ...
       if (mpi_id == 0) then
          write(*,'(A,F0.2,A,F0.2,A,F0.2,A,F0.2,A)') &
    'The subsidy multipliers are s=(', s_vec(1), ', ', s_vec(2), &
    '), which is equivalent to: r=(', s_vec(2), ', ', s_vec(1)-s_vec(2), ')'
       end if
    else
  ...
  subroutine read_model_params(param0)
    ...
    if (validation == 1) then
       open(1, file = trim(filepath)//'/code/fortran_model/common_f90s/initial_param_guess_validation.txt', status = 'old')
       !open(1, file = 'initial_param_guess_validation.txt', status = 'old')
    else if (purpose == -1 .or. purpose == -2) then
       open(1, file = trim(filepath)//'/code/fortran_model/common_f90s/initial_param_guess_se_perturb.txt', status = 'old')
       !open(1, file = 'initial_param_guess_se_perturb.txt', status = 'old')
    else
       open(1, file = trim(filepath)//'/code/fortran_model/common_f90s/initial_param_guess.txt', status = 'old')
  ...
  subroutine read_theta()
    integer :: i
    if (validation == 1) then
       open(1, file = trim(filepath)//'/code/fortran_model/common_f90s/initial_theta_validation.txt', status = 'old')
       !open(1, file = './initial_theta_validation.txt', status = 'old')
    else
       open(1, file = trim(filepath)//'/code/fortran_model/common_f90s/initial_theta.txt', status = 'old')
    ...
    if (purpose == -1 .or. purpose == -2) D_sofar = D_sofar + 0.01
  ...
  subroutine read_weight_matrixes()
    ...
    open(1, file = trim(filepath)//'/code/fortran_model/common_f90s/moment_weight_factors.txt', status = 'old')
    ...
    !adjust weights
    do i=1, naux
       if (purpose == -1 .or. purpose == -2) auxvar(i) = 1/weight_mat(i,i) ! hijack var for SE calc
       weight_mat(i,i) = weight_mat(i,i) * aux_weight_factor(i,1)
    end do
    do i=1, naux_omega
       if (purpose == -1 .or. purpose == -2) aux_omegavar(i) = 1/omega_weight_mat(i,i) ! hijack var for SE calc
       omega_weight_mat(i,i) = omega_weight_mat(i,i) * omega_weight_factor(i,1)
    end do
    do i=1, naux_other
       if (purpose == -1 .or. purpose == -2) aux_othervar(i) = 1/other_weight_mat(i,i) ! hijack var for SE calc
...
!  end subroutine adjust_weights
  ...
  function median(vec) result(med_val)
    ...
    if (modulo(len,2) == 1) then
       ...
       med_idx = len / 2
       med_idx2 = med_idx + 1
       med_val = (vec_sorted(med_idx) + vec_sorted(med_idx2)) / 2
    end if
  end function median

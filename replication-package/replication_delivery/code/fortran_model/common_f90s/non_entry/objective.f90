...
contains
  ...
  function objective_function(param0_in) result(L)
    ...
    if (purpose .ne. -2) then
       ...
       k=k+1
    else !purpose == -2, i.e., perturbation code
       ! param0_in is from the solver, then we scale by our custom step size
       ! initial_param never change.
       j=1 !index for subset of included parameters
       k=1 !index for all parameters
       ...
       do i = 1, nalpha
          ...
          ! NO TRANSFORMATIONS NEEDED FOR PERTURB:
          k = k + 1
       end do
       ! NO TRANSFORMATIONS NEEDED FOR PERTURB:
    ...
    if (nomovecost_experiment == 1) then
       ! Transformations for 'CASE' comparative static experiments (i.e. no move cost)
       alpha_in(6:13) = alpha_in(6:13) * 0
       !sigma_in = sigma_in * 0.8**1
    end if
...
11        format(I4, 8F20.8)
    ...
    else if (offer_eqbm == 1 .and. purpose > 0) then 
       !call write_latest_theta()
       print*, "not writing latest theta for replication package, see obj.f90"
    end if

  ...
  subroutine run_inner_loop_one(pr_model_match,model_match_copies, pr_model_Wgrid, &
    ...
    if (teachOff_decision == 1) then
       if (offer_eqbm == 1 .and. skip_inner_loop == 0) then
          !
          if(nround<=sum(incl,dim=1)+2  .or. purpose == -1 .or. purpose == -2) then
             theta = theta0
          else
             theta = theta_sofar
          endif
          ...
          if(nround==1 .and. purpose > 0) then
             theta0 = best_theta
          endif
          theta       = best_theta
          !print*, "theta, after inner"
          !print*, theta 
          H           = h_logit(theta)
          ...
          if (purpose == -1 .or. purpose == -2) theta = theta0 !just in case, this makes sure it would start w/ right belief
...
!!!
  ...
  subroutine simulate_equilibrium(alpha_in, beta_in, gamma_in, sigma_in, sigmaD_in,&
    ...
    double precision, allocatable, dimension(:,:) :: econ_main_theta
    !double precision, dimension(50, 20) :: econ_main_theta
    
    double precision, dimension(ntheta) :: abspctdiff

    ! A little bit of a weird setup that was hacked together
    ! For purpose <=10, i.e., NO counterfactual money, belief parameters
    !    21 and 22 don't matter. Those coefficients are turned off so those #s
    !    can be whatever
    ! (In general, theta0 always has 22 size.)
    ! For purpose > 10, params 21 and 22 start to matter. Params 1-20 are read-in
    !    from what they were for the baseline non-money intervention. Params 21/22
    !    then get passed through from initial_belief_guess.txt, whereas 1-20
    !    are overwritten economy-by-economy.
    if (purpose > 10) then
       allocate(econ_main_theta(50, 20))
    else
       allocate(econ_main_theta(50,22))
    end if
    ...
    close(1)
    ! Late addition
    open(1, file = 'result/teacher_welfare.txt', status='replace')
    write(1,*) ' economy_id', ' aggregate_teacher_welfare'
    close(1)
    ...
    do m=1, Neconomy
       ...
       print*, 'working on eqlbm for economy: ', m, ' of ', Neconomy
       ! for cf, initial guess will now come from 50 econ (for main only have 20 of 22)
       if (purpose > 10) then
          theta0_wage = econ_wage_theta(economy_idx,:)
          theta0(1:20) = econ_main_theta(economy_idx,:)
       end if
       if (purpose <=10 .and. nomovecost_experiment == 0) then
       ...
       do
          ...
          ! need to introduce something to avoid NAs here b/c we were making thetas == 0 so can't do pct change like this
          abspctdiff = 0
          do j =1, ntheta
             !if theta is zero, don't use as denominator
             if (abs(theta(j)) < epsilon(theta(j))) then
                !if theta_prev is not zero, can use that instead
                if (abs(theta_prev(j)) > epsilon(theta_prev(j))) then
                   abspctdiff(j) = abs((theta_prev(j) - theta(j))/theta_prev(j))
                end if
             !else theta is ok as denominator
             else
                abspctdiff(j) = abs((theta_prev(j) - theta(j))/theta(j))
             end if
          end do

          
          !if( sum(abs((theta_prev - theta)/theta))/ntheta + &
          if (sum(abspctdiff / ntheta) + &
             ...
             print*, sum(abspctdiff/ntheta), sum(abs((theta_wage_prev - theta_wage)))/ntheta_wage
             exit
          else
             print*, 'still working on convergence'
             !print*, sum(abs((theta_prev - theta)/theta))/ntheta, sum(abs((theta_wage_prev - theta_wage)))/ntheta_wage
             print*, sum(abspctdiff/ntheta), sum(abs((theta_wage_prev - theta_wage)))/ntheta_wage
          end if
  ...
  function equilibrium_objective(arg1, arg2) result(D)
    ...
    if (economy_output_indicator == 1) then
       ...
       ! Addition: Teacher welfare
       open(1,file='result/teacher_welfare.txt', action='write',position='append')
       ! sum teacher utility, across all offers/probability of being in that district
       write(1,*) economy_idx, sum(H_target * offer_eqm_glob * uteach)
    ...
    do j=1, rec_count/nteach
       ...
       do a1=1, nWgrid1
          do a2=1,nWgrid2
             ...
             ! Need this wedge because teachers will see subsidy in their decision,
             !   but it is outside of district budgets otherwise

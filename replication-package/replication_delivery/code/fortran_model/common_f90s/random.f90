! Taken from Salvador Code
!
! Edited slightly to remove warnings in compilation
! (nonconforming tabs, integer conversion, unused variable)
MODULE RANDOM
  USE NRUTIL, ONLY : NRERROR,NR_SMALL,NR_BIG,NR_EPS
  IMPLICIT NONE
...
CONTAINS
  ...
  SUBROUTINE Set_Seed(k1,k2,k3,k4)
    ...
    DO  i = 126, 1, -1
       ...
       zigkn(i+1) = int((zigdn/zigtn)*zigm1)
       zigtn = zigdn
    ...
    DO  i = 254, 1, -1
       ...
       zigke(i+1) = int(zigm2 * (zigde/zigte))
       zigte = zigde
       zigfe(i) = EXP( -zigde )
       zigwe(i) = zigde/zigm2
    END DO
  ...
  REAL(8) FUNCTION Sample_Gamma(ain,b)
    ...
    DO
       ...
       IF (u < 1.0d0 - 0.0331d0*x**4) THEN
          ...
          EXIT
       ELSE IF (LOG(u) < 0.5d0*(x**2) + d*(1.0d0 - v + LOG(v))) THEN
          Sample_Gamma = correction*d*v
          EXIT
       END IF
    END DO
  ...
  REAL(8) FUNCTION Sample_Double_Truncated_Normal_Geweke(mu,var,a,b)
    ...
    REAL(8),PARAMETER :: t1=0.375D0,t2=2.18D0,t3=0.725D0
    LOGICAL :: lflip
    INTEGER :: j
...
!    END IF
  ...
  FUNCTION P_Cholesky_Ran(AIN)
    ...
    REAL(8) :: A(SIZE(AIN,1),SIZE(AIN,2)),p(SIZE(AIN,1)),P_Cholesky_Ran(SIZE(AIN,1),SIZE(AIN,2))
    INTEGER :: i,j,n
    REAL(8) :: summ
    IF (SIZE(AIN,1).NE.SIZE(AIN,2)) CALL NRERROR('matrix is not square: P_Cholesky_Ran')
    A=AIN
    P_Cholesky_Ran=0.0d0
    n=SIZE(A,1)
    DO i=1,n
       summ=A(i,i)-DOT_PRODUCT(A(i,1:i-1),A(i,1:i-1))
       IF (summ <= 0.0) CALL NRERROR('P_Cholesky_Ran failed')
       p(i)=SQRT(summ)

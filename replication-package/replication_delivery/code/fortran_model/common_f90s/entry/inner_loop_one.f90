...
!!!
  ...
  subroutine simulate_equilibrium(alpha_in, beta_in, gamma_in, sigma_in, sigmaD_in,&
    ...
    double precision, dimension(50, 22) :: econ_main_theta

    integer :: econ_specific_init_beliefs
    ...
    !For both entry baseline and counterfactual, the first period is just
    !   the main baseline eqbm, so start from those converged beliefs always.
    econ_specific_init_beliefs = 1
    if (purpose >= 10 .and. t_period <= max_period_initbeliefs) then
       call load_period_beliefs(t_period, econ_wage_theta, econ_main_theta)
    else
       ! Otherwise, eqbm will start from estimation beliefs (no economy-by-economy)
       econ_specific_init_beliefs = 0
    end if
    ...
    do m=1, Neconomy
       ...
       if (econ_specific_init_beliefs == 1) then
          theta0_wage = econ_wage_theta(economy_idx,:)
          theta0      = econ_main_theta(economy_idx,:)
       else if (t_period > 1) then
          ! beliefs will start from last period's converged beliefs
          theta0_wage = wage_theta_period(economy_idx,:)
          theta0 = theta_period(economy_idx,:)
       else if (t_period == 1) then
          ! beliefs will just start from original theta0, i.e. estimation beliefs
       else
          ! in enter/exit and forward iteration we need to be careful abt beliefs
          print*, 'dont think we should have gotten here... investigate'
          call exit(1)
       end if
       ! beliefs for entry/exit forward iter are read in econ-by-econ
       !if (purpose <= 10) then
...
!       close(1)
  ...
  ! for each period, this will load in the economy-by-economy beliefs
  subroutine load_period_beliefs(tp, wage_th, main_th)
  integer, intent(in) :: tp
  double precision, intent(out) :: wage_th(:,:), main_th(:,:)
  character(len=64) :: fw, fm
  character(len=8)  :: sfx

  if (tp == 1) then
     sfx = ''
  else
     write(sfx,'(A2,I0)') '_t', tp
  end if

  fw = 'wagebelief_50'//trim(sfx)//'.txt'
  fm = 'mainbelief_50'//trim(sfx)//'.txt'

  open(1,file=trim(fw),status='old'); read(1,*) wage_th; close(1)
  open(1,file=trim(fm),status='old'); read(1,*) main_th; close(1)
  
end subroutine load_period_beliefs

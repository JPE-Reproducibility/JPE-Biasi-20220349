...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! GAMMA SPECIAL FUNCTIONS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  REAL(8) FUNCTION NR_gammln(xx)
    IMPLICIT NONE
    REAL(8), INTENT(IN) :: xx
    REAL(8) :: tmp,x
    REAL(8) :: stp = 2.5066282746310005d0
    REAL(8), DIMENSION(6) :: coef = (/76.18009172947146d0,-86.50532032941677d0, &
         24.01409824083091d0,-1.231739572450155d0,0.1208650973866179d-2,-0.5395239384953d-5/),aux
    INTEGER :: k
    IF (xx<0.0d0) CALL NRERROR('argument has to be postive: NR_gammln')
    x=xx
    tmp=x+5.5d0
    ...
    DO k=2,6
       aux(k) = aux(k-1)+1.0d0
    END DO
    NR_gammln=tmp+LOG(stp*(1.000000000190015d0+&
         SUM(coef(:)/aux))/x)
  END FUNCTION NR_gammln
  ...
  REAL(8) FUNCTION NR_gser(a,x,gln)
    ...
    if (present(gln)) then
       ...
       NR_gser=summ*exp(-x+a*dlog(x)-NR_gammln(a))
    end if
  ...
  REAL(8) FUNCTION NR_gcf(a,x,gln)
    ...
    if (present(gln)) then
       ...
       NR_gcf=exp(-x+a*log(x)-NR_gammln(a))*h
    end if
  END FUNCTION NR_gcf

...
* Locate the replication root when launched from either the repository root
* or this script's own code/auxiliary_regressions directory.
local launch_dir "`c(pwd)'"
capture confirm file "`launch_dir'/code/auxiliary_regressions/master.do"
if _rc {
    capture confirm file "`launch_dir'/master.do"
    if !_rc {
        quietly cd "../.."
        local launch_dir "`c(pwd)'"
    }
...
if _rc {
    ...
    di as error "Start Stata in the replication root or code/auxiliary_regressions."
    exit 601
}

global rootpath "`launch_dir'"
global code = "$rootpath/code/"
global confdata = "$rootpath/data/"
...
global synthdata = "$rootpath/synthetic_data/"
global out = "$rootpath/out/"
...
* Figure B9 
do $code/Figure_B9/Figure_B9.do // see R code in Figure_B9 folder for generation of the simulation data for this figure
